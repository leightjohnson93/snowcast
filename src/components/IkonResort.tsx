...
import { Theme, Resort } from '../interfaces'

const IkonResort: React.FC<{
  resort: Resort
  theme: Theme
  maxWeightedSnowfall: number
}> = ({ resort, theme, maxWeightedSnowfall }) => {
  const [expanded, setExpanded] = useState(false)
  const { pastSnow, primaryRegion, snowcone, weightedSnowfall, name } = resort
  const { sum3, snow0day } = pastSnow
  const { country } = primaryRegion
  const { active_date_epoch } = snowcone
            ...
            <Grid container justify="space-around">
              <Grid item>
                <List>
                  ...
                  <ListItem>
                    <ListItemText primary={`${sum3}"`} secondary="3 Days" />
                  </ListItem>
                  {/* <ListItem>
                    <ListItemText
                      primary={`${last7Days}"`}
...
export default withTheme()(IkonResort)
